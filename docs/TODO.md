...
1. **v2 实拍终审**（samuel 双镜头：采购 30 秒 → 评委细看）：`/m2/atlas-paper`（生产候选）+ `/proof` 彩蛋页。通过 → M2 收官（v2 转正为主站首页）；不通过 → 拍回具体问题继续修。

## 已排期的技术项（非阻塞，待触发）

| 项 | 内容 | 前置条件 / 触发点 |
|---|---|---|
| View Transitions | Astro 内置客户端路由 + 跨页过渡动画：切语言/换页不再整页白闪，观感接近单页应用；URL 与 SEO 不变 | **需先重构页面脚本生命周期**——现有 `<script>` 模块只在首次加载执行，客户端跳转后 globe/交互会失效，必须改为监听 `astro:page-load` 重新初始化。建议主体设计定稿后单独做一轮，不与设计改动夹带 |
| 站点导航与功能回装 | 刊头式导航（含语言切换主入口）、双 CTA 弹窗表单接回、页脚导航列、锚点与移动菜单 | v2 设计终审通过后 |
